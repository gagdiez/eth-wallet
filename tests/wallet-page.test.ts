...
const mocks = vi.hoisted(() => ({ receive: vi.fn(), choose: vi.fn(), signIn: vi.fn(), connect: vi.fn(), send: vi.fn() }));
vi.mock('../src/sign-page', () => ({ receiveRequest: mocks.receive }));
vi.mock('../src/appkit', () => ({ chooseAppKitWallet: mocks.choose }));
...
beforeEach(() => {
  ...
  mocks.choose.mockResolvedValue({}); mocks.signIn.mockResolvedValue(account); mocks.connect.mockResolvedValue(account); mocks.send.mockResolvedValue(['outcome']);
});
...
describe('wallet page authorization', () => {
  ...
  it('revokes the requesting site without opening or disconnecting AppKit', async () => {
    const { OriginGrants } = await import('../src/grants');
